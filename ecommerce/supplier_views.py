...
from ecommerce.forms import sup_forms
from ecommerce.models import Supplier, Order, Product, Category, Order_status
from ecommerce.supplier_functions import (
    pull_supplier_orders,
    check_sup_user_event,
    product_belong_supplier,
    add_product,
)
...
def sup_orders():
    ...
    if (
        request.method == "POST"
        and "make_shipment" in request.json
        and request.json["make_shipment"] == True
    ):
...
def sup_product_data(pid):
    ...
    if request.method == "POST" and request.json and "page" in request.json:
        ...
        data["reviews"] = data["product"].reviews.paginate(page, PER_PAGE, False)
        return render_template("/suppliers/sup_product_data.html", **data)

    data["reviews"] = data["product"].reviews.paginate(1, PER_PAGE, False)
    return render_template("/suppliers/sup_product_data.html", **data)
...
def suppliers_main():
    ...
    if (
        data["forms"]["supplier_add_product"].add_product.data
        and data["forms"]["supplier_add_product"].validate_on_submit()
    ):
