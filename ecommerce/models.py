...
class Cart(db.Model, UserMixin):
    ...
    def __init__(
        self, buyer_id, product_id, qty, status=1, buy_now=False, buyer_message=False
    ):
...
class Buyer(db.Model, UserMixin):
    ...
    def __init__(
        self, email, username, password, name="N/A", address="N/A", photo="default.jpg"
    ):
    ...
    def cancel_cary_item(self, cart_id):
        if (
            not Cart.query.get(cart_id)
            or not Cart.query.get(cart_id).buyer_id == self.id
        ):
    ...
    def get_reviews(self, count=False):
        ...
        response["get_reviews"] = (
            ...
            .join(Order, Reviews.order_id == Order.id)
...
class Supplier(db.Model, UserMixin):
    ...
    def get_orders(self, status=None, pid=None, count=False, income=False):
        ...
        if income:
            response["income"] = (
                ...
                .filter(Order.supplier_id == self.id)
                .all()[0][0]
            )
    ...
    def get_supplier_reviews(self, avg=False):
        ...
        if avg:
            response["avg"] = (
                db.session.query(db.func.avg(Reviews.stars))
...
class Product(db.Model, UserMixin):
    ...
    def get_info(self):
        ...
        response["orders"] = self.get_product_orders(
            get_product_orders=False, sum_orders=True, sum_units=True
        )
    ...
    def get_product_orders(
        self, get_product_orders=True, count_orders=False, count_units=False
    ):
        response = dict()
        if get_product_orders:
            response["get_product_orders"] = (
        ...
        if count_units:
            response["count_units"] = (
                db.session.query(db.func.sum(Order.qty))
                .join(Product, Product.id == Order.product_id)
                .filter(Order.product_id == self.id)
                .all()[0][0]
            )
    ...
    def get_review(self, get_review=True, avg=False, count=False):
        ...
        if count:
            response["count"] = (
                db.session.query(db.func.count(Reviews.id))
                .join(Order, Order.id == Reviews.order_id)
                .filter(Order.product_id == self.id)
                .all()[0][0]
            )
    ...
    def open_order_count(self):
        return (
            ...
            .join(Order_status)
            .filter(Order.product_id == self.id)
            .filter(Order_status.is_open == True)
            .count()
        )
...
class Order(db.Model, UserMixin):
    ...
    def __init__(
        self, product_id=None, buyer_id=None, qty=None, status=1, cart_item=False
    ):
        ...
        self.supplier_id = Supplier.query.get(
            (Product.query.get(self.product_id).supplier_id)
        ).id
        self.order_time = datetime.utcnow()
        self.last_change_time = datetime.utcnow()
        self.status = status
...
class Reviews(db.Model, UserMixin):
    ...
    def get_review_buyer(self):
        ...
        if type(order) == int:
            buyer_id = (
                db.session.query(Order.buyer_id).filter(Order.id == order).first()
            )
