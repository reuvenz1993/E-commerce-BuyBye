...
def account():
    ...
    if request.args.get("focus_order") and int(request.args.get("focus_order")) in (
        order.id for order in current_user.orders
    ):
...
def my_cart():
    ...
    if args.get("type") == "update_buyer_message":
        return jsonify(
            update_buyer_message(
                item_id=args.get("item_id"), buyer_message=args.get("buyer_message")
            )
        )
    ...
    if args.get("type") in cart_functions:
        ...
        data["action"] = {
            "success": res,
            "type": args.get("type"),
            "item": Cart.query.get(args.get("item_id", False)),
        }
...
def results():
    ...
    results = search(**keyword_args).paginate(
        1, PER_PAGE, False
    )  # proform search with params, if keyword_args is None - return all products

    categorys = Category.query.all()

    return render_template(
        "results.html", results=results, categorys=categorys, **forms, **keyword_args
    )
