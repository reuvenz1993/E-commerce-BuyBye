...
def product_belong_supplier(pid, supplier_id):
    if (
        not Product.query.get(pid)
        or not Product.query.get(pid).supplier_id == supplier_id
    ):
        return False
    return True
...
def add_product(supplier_add_product):
    ...
    new_product = Product(
        name=supplier_add_product.name.data,
        supplier_id=supplier_id,
        price=supplier_add_product.price.data,
        category=supplier_add_product.category.data,
        desc=supplier_add_product.desc.data,
        brand=supplier_add_product.brand.data,
        picture=picture_fn,
...
def check_sup_login(sup_loginform):
    supplier_logging = Supplier.query.filter_by(
        username=sup_loginform.username.data
    ).first()
    if supplier_logging is not None and supplier_logging.check_password(
        sup_loginform.password.data
    ):
        session["supplier_username"] = sup_loginform.username.data
        session["supplier_id"] = (
            Supplier.query.filter_by(username=sup_loginform.username.data).first().id
        )
        return True
...
def sup_signup(sup_signupform):
    try:
        new_supplier = Supplier(
            sup_signupform.email.data,
            sup_signupform.username.data,
            sup_signupform.password.data,
            sup_signupform.name.data,
            sup_signupform.type_of.data,
            sup_signupform.address.data,
        )
...
def check_sup_user_event(forms):
    if (
        forms["sup_loginform"].supplier_login.data
        and forms["sup_loginform"].validate_on_submit()
    ):
    ...
    if (
        forms["sup_signupform"].supplier_signup.data
        and forms["sup_signupform"].validate_on_submit()
    ):
